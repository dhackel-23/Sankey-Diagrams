...
def optimise_node_order_model(model_inputs, group_nodes=False, crossings__area=False):
    ...
    # Calculate total crossing area manually
    total_crossing_area = 0

    for (u1v1, u2v2), c_var in c_main_main.items():
        if c_var.x >= 0.99:  # crossing happens
            total_crossing_area += edge_weight[u1v1] * edge_weight[u2v2]

    for (u1v1, u2wp), c_var in c_exit_forward.items():
        if c_var.x >= 0.99:
            total_crossing_area += edge_weight[u1v1] * edge_weight[u2wp]

    for (u1wp1, u2wp2), c_var in c_exit_exit.items():
        if c_var.x >= 0.99:
            total_crossing_area += edge_weight[u1wp1] * edge_weight[u2wp2]

    for (u1v1, wpv2), c_var in c_return_forward.items():
        if c_var.x >= 0.99:
            total_crossing_area += edge_weight[u1v1] * edge_weight[wpv2]

    for (wp1v1, wp2v2), c_var in c_return_return.items():
        if c_var.x >= 0.99:
            total_crossing_area += edge_weight[wp1v1] * edge_weight[wp2v2]

    # After solving, collect crossing info
    crossing_detection = {}
    crossing_areas = {}

    for (u1v1, u2v2), c_var in c_main_main.items():
        if c_var.x >= 0.99:
            crossing_detection[(u1v1, u2v2)] = 1
            crossing_areas[(u1v1, u2v2)] = edge_weight[u1v1] * edge_weight[u2v2]

    for (u1v1, u2wp), c_var in c_exit_forward.items():
        if c_var.x >= 0.99:
            crossing_detection[(u1v1, u2wp)] = 1
            crossing_areas[(u1v1, u2wp)] = edge_weight[u1v1] * edge_weight[u2wp]

    for (u1wp1, u2wp2), c_var in c_exit_exit.items():
        if c_var.x >= 0.99:
            crossing_detection[(u1wp1, u2wp2)] = 1
            crossing_areas[(u1wp1, u2wp2)] = edge_weight[u1wp1] * edge_weight[u2wp2]

    for (u1v1, wpv2), c_var in c_return_forward.items():
        if c_var.x >= 0.99:
            crossing_detection[(u1v1, wpv2)] = 1
            crossing_areas[(u1v1, wpv2)] = edge_weight[u1v1] * edge_weight[wpv2]

    for (wp1v1, wp2v2), c_var in c_return_return.items():
        if c_var.x >= 0.99:
            crossing_detection[(wp1v1, wp2v2)] = 1
            crossing_areas[(wp1v1, wp2v2)] = edge_weight[wp1v1] * edge_weight[wp2v2]
    ...
    banded_order = [[] for _ in range(len(node_layer_set))]
    ...
    if crossings__area:
        return banded_order, crossing_detection, crossing_areas
    else:
        return banded_order


def optimise_node_order(sankey_data, group_nodes=False, crossings__area=False):
    model = model_inputs(sankey_data, group_nodes=group_nodes)

    if crossings__area:
        opt_order, crossings, crossing_areas = optimise_node_order_model(model, group_nodes=group_nodes, crossings__area=True)
        new_sankey_data = evolve(sankey_data, ordering=opt_order)
        return new_sankey_data, crossings, crossing_areas
    else:
        opt_order = optimise_node_order_model(model, group_nodes=group_nodes, crossings__area=False)
        new_sankey_data = evolve(sankey_data, ordering=opt_order)
...
def straightness_model(sankey_data):
    """Build model to optimise node straightness."""

    order = sankey_data.ordering.layers
    node_layer_set = [[] for _ in range(len(order))]
    node_band_set = [[] for _ in range(len(order[0]))]
    node_dict = {}
    ...
    for flow in flows:
        sl = node_dict[flow.source]['layer']
        tl = node_dict[flow.target]['layer']
...
def optimise_hybrid_model(straightness_model, 
    ...
    c_main_main = { (u1v1,u2v2): m.add_var(var_type=BINARY)
                    for Ek in edges for u1v1 in Ek for u2v2 in Ek if u1v1 != u2v2 }
